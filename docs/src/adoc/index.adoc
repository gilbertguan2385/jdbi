...
The `jdbi3-json` module adds a `@Json` qualified type that stores arbitrary Java objects as JSON data in your database.
It also registers default Arguments and Mappers that are based on `String/varchar` handling.
...
[WARNING]
If your database has custom JSON support, you *must* install its plugin
_after_ installing a JSON backing implementation. Otherwise the default factories will be used.
...
An ArgumentFactory qualified for `@Json String` will then be called to store the JSON data.

Mapping works just the same way, but in reverse: an output type qualified as `@Json T` will be fetched from a
`@Json String` ColumnMapper, and then passed through the `JsonMapper`.

[TIP]
Our PostgresPlugin provides qualified factories that will
bind the `@Json String` to `json` or `jsonb`-typed columns.
